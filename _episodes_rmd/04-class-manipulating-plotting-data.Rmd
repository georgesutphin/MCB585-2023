...
> *Note: there is no built-in function for calculating standard error of mean (SEM), but this can be easily calculated using `sd(x)/sqrt(x)`. There are also packages that have functions available (e.g. `pastecs`).*
>
> Miscellaneous:

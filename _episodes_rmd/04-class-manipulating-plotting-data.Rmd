...
# calculate the max number of aneurisms for subject 1 across 
# time points
max(sub1)
```
...
> 
> There is a mixture of lower case (m, f) and upper case
...
Now look inside your results folder (Windows and Mac differ here, see below). If the `jpeg()` function worked correctly and our directories were correctly specified, you should now see a new file called *petal-width-length-comparison.jpg*. What happens when you try to open this file?
...
* In the current version of R, the `.jpg` file will only be created in Windows (not MacOS). In MacOS, the file will only be created after executing the `dev.off()` command below. In both Windows and MacOS, the `pdf()` function discussed next will create the file immediately.
...
# achieve this same effect by first converting the character to a factor,
# then converting the factor to numeric: as.numeric(as.factor(x))

# Taking advantage of this, we can directly index our color list and set 
# up a vector the length of iris$Species with a unique color value for 
# each species name
